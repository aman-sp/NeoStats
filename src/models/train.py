...
def train_and_evaluate_all_models(
    ...
    # 8. Calibration Assessment using a validation-fit sigmoid calibrator.
    ...
    val_features = np.clip(raw_val_proba.reshape(-1, 1), 1e-6, 1 - 1e-6)
    calibration_model = LogisticRegression(max_iter=1000, solver='lbfgs')
    calibration_model.fit(val_features, y_val)

    calib_val_proba = calibration_model.predict_proba(np.clip(raw_val_proba.reshape(-1, 1), 1e-6, 1 - 1e-6))[:, 1]
    calib_test_proba = calibration_model.predict_proba(np.clip(raw_test_proba.reshape(-1, 1), 1e-6, 1 - 1e-6))[:, 1]
    calib_brier = float(brier_score_loss(y_test, calib_test_proba))

    calibration_adopted = False
    final_inference_model = best_model
    final_val_proba = raw_val_proba

    if calib_brier < raw_brier:
        ...
        final_inference_model = __import__('src.models.predict', fromlist=['CalibratedRiskModel']).CalibratedRiskModel(best_model, calibration_model)
        final_val_proba = calib_val_proba
    else:
