...
class CreditRiskExplainer:
    ...
    def __init__(self):
        ...
        # Handle wrapped calibration models if adopted
        underlying_model = getattr(self.model, 'base_model', self.model)
        if hasattr(self.model, 'calibrated_classifiers_') and len(getattr(self.model, 'calibrated_classifiers_', [])) > 0:
